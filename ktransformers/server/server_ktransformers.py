...
def start_model_server():
    ...
    try:
        ...
        cmd = [
            "cd", "/models/ktransformers", "&&",
            "python", "-u", "-m", "ktransformers.server.main",  # Add -u for unbuffered output
            "--gguf_path", "/models/deepseek/gguf",
            "--model_path", "/models/deepseek/config",
    ...
    async def get_model_info():
        ...
        try:
            if model_process is None or model_process.poll() is not None:
                return {
                    "status": "Model server not running",
